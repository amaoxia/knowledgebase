...
// Tree object
function dTree(objName,appPath,treetype) {
	this.config = {
		target					: null,
		folderLinks			: true,
		useSelection		: true,
		...
		//新增checkbox是否显示(,0file,1checkbox,2radio)
		check:					treetype,
...
{
	var imgBasePath=this.imgBasePath+"/js/dtree/";
...
dTree.prototype.node = function(node, nodeId) 
{
  ...
  if(node.pid==-1)
  {
	...
	else
	{
		 if(this.config.check==0){
		    //str+= ' value="'+node.name+'" id="c'+  this.obj.name +'" onclick="'+this.obj.name+'.doClick('+nodeId+')"/>';
		}		
		else if(this.config.check==1)//新增checkbox表单元素
		{
	  ...
	  }else if(this.config.check==2){
	  	str+= '<input type="radio"  id="c'+  this.obj.name + nodeId + '" name=" '+'grdo" ';
	     
	     str+=' onclick=tree.showNodeInfo("'+ this.obj.name+ nodeId+'","' + node.id +'","'+nodeId+'","'+ node.title +'","'+ node.param + '")';
  ...
  //str += node.title;
	str += '<a href="#" onclick=tree.showNodeInfo("'+ this.obj.name+ nodeId+'","' + node.id +'","'+nodeId+'","'+ node.title +'","'+ node.param + '")>'+node.title+'</a>'
	str += '</div>';
...
dTree.prototype.initDTreeNode1 = function() {
	var arrdata = this.data.split(";");
	for(var i=0;i<arrdata.length;i++) {
		if(arrdata[i] == null || arrdata[i] == "") continue;
	 	var arrs = arrdata[i].split(",");
	    this.add(arrs[0], arrs[1], arrs[0], '#', arrs[2], arrs[3]);	   
	 }
	 this.obj.innerHTML = this;
}
...
dTree.prototype.doChecked=function(nodeId)
{
  ...
  if(this.config.isCheckedParent){
	 this.setToParentChecked(nodeId,cs);
  }else{
  	this.setToParentChecked2(nodeId,cs);
  }
 // if(cs==false){//modify by 胡俊 取消不选中节点时父节点也不选中
    //this.setToUpChecked(node); 
  //}  
  
  //选中节点，如果其平级节点状态相同，则改变父节点状态，并传递到根节点
}

dTree.prototype.setToParentChecked=function(nodeId,nstatus){
	  var parent = this.getParent(nodeId);
	  if(parent){
	  	if(nstatus==false){
		 	 var childrens = this.getChildrens(parent.id);
		 	 for(var i=0; i<childrens.length; i++){
		 	 	var o = childrens[i];
		 	 	if(document.getElementById("c"+this.obj.name+o._ai).checked==true){
		 	 		return;
		 	 	}
		 	 }
	  	}
	 	 var pNode = document.getElementById("c"+this.obj.name+parent._ai);
	 	 if(pNode){
	 	 	pNode .checked=nstatus;
	 	 	this.setToParentChecked(parent._ai,nstatus);
	 	 }
  }
}

dTree.prototype.setToParentChecked2=function(nodeId,nstatus){
	  var parent = this.getParent(nodeId);
	  if(parent){
	  		var pNode = document.getElementById("c"+this.obj.name+parent._ai);
	  		if(nstatus==false){
	  			 if(pNode){
			 	 	pNode.checked=nstatus;
			 	 	this.setToParentChecked2(parent._ai,nstatus);
			 	 }
	  		}else{
	  		 var childrens = this.getChildrens(parent.id);
		 	 for(var i=0; i<childrens.length; i++){
		 	 	var o = childrens[i];
		 	 	if(document.getElementById("c"+this.obj.name+o._ai).checked==false){
		 	 		return;
		 	 	}
		 	 }
		 	  if(pNode){
...
//设置父节点选中状态
dTree.prototype.setToUpChecked=function(node, nstatus)
{	
  ...
  for (var m=0; m<len; m++) 
  {
  	if (this.aNodes[m].id == node.pid) {
  		  ...
  		  {
	        document.getElementById("c"+this.obj.name+m).checked=nstatus;
...
dTree.prototype.getParent=function(nodeId){
	 var node = this.aNodes[nodeId]; 
	 if(node._p)return node._p;
	 return null;
}

dTree.prototype.getChildrens=function(id){
    var len =this.aNodes.length;
    var childrens = new Array();
    for (var m=0; m<len; m++) {
	   	if (this.aNodes[m].pid == id) {
	  		 childrens.push(this.aNodes[m]);
	    }
   }
    return childrens;
}

dTree.prototype.showNodeInfo=function(nodename, nodeId, id, nodeTitle, param){
	if(id=="0"){
		window.parent.showNodeInfo(nodeId, id);
		return;
	}
	...
	if(nodeId==''){
		...
		array = nodeId.split(",");
   		...
   		if(nodeId==""){
			...
			window.parent.showNodeInfo(nodeId, id, nodeTitle, param);
 		}
	}
}